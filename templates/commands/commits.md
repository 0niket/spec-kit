...
2. **Load context**: Read from FEATURE_DIR:
   ...
   - **Required**: `memory/constitution.md` (for repetitive tasks)
   - **Optional**: plan.md (for context on project structure)

3. **Parse constitution for repetitive tasks**: Run `scripts/bash/parse-constitution.sh` on the constitution file to get JSON output of required repetitive tasks (TDD, linting, verification).
